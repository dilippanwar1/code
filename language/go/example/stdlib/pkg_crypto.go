...
// or text blobs. For example, the git revision control uses SHA1s extensively
// to identify versioned files and directories. Here's how to compute SHA1
// hashes in Go.
...
func main() {
	...
	// `Write` expects bytes. If you have a string `s`, use `[]byte(s)` to
	// coerce it to bytes.
	h.Write([]byte(s))
