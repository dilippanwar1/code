...
func main() {
	// Client.Transport must implement RoundTripper interface, which contains only
	// one method: RoundTrip(*Request) (*Response, error). It is called from within
	// net/http#send function where a HTTP request is issued. The RoundTrip method
	// does some of the following stuffs:
	// 1. Validate request, e.g. request schema should be http or https;
	// 2. Low level connection, optionlly tls handshake, e.g. net.Dail("tcp", addr);
	// 3. Set common HTTP request header;
	// 4. Write request, read response.
	client := &http.Client{
...
// Copid from "github.com/pkulak/simpletransport/simpletransport"
