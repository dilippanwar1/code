...
func logPanic(r interface{}) {
	...
	fmt.Printf("Recovered from panic: %#v (%v)\n%v", r, r, callers)
}
...
func isInterfaceUp(intf *net.Interface) bool {
	...
	if intf.Flags&net.FlagUp != 0 {
		fmt.Printf("Interface %v is up", intf.Name)
		return true
	}
	return false
}
...
func getFinalIP(addrs []net.Addr) (net.IP, error) {
	if len(addrs) > 0 {
		for i := range addrs {
			fmt.Printf("Checking addr  %s.", addrs[i].String())
			...
			if ip.To4() != nil {
				if !ip.IsLoopback() {
					fmt.Printf("IP found %v", ip)
					return ip, nil
				} else {
					fmt.Printf("Loopback found %v", ip)
				}
			} else {
				fmt.Printf("%v is not a valid IPv4 address", ip)
			}
...
func getIPFromInterface(intfName string, nw networkInterfacer) (net.IP, error) {
	...
	if isInterfaceUp(intf) {
		...
		fmt.Printf("Interface %q has %d addresses :%v.", intfName, len(addrs), addrs)
		finalIP, err := getFinalIP(addrs)
		if err != nil {
			return nil, err
		}
		if finalIP != nil {
			fmt.Printf("valid IPv4 address for interface %q found as %v.", intfName, finalIP)
			return finalIP, nil
		}
	}
...
func chooseHostInterfaceNativeGo() (net.IP, error) {
	...
	fmt.Printf("Choosing interface %s (IP %v) as default", intfs[i].Name, ip)
	return ip, nil
}

// ChooseHostInterface is a method used to fetch an IP for a daemon.
...
func main() {
	fmt.Println(ChooseHostInterface())
}
