...
int main(int argc, char **argv)
{
  ...
  */
  ...
  /* grab a packet from descr (yay!)
     ...
     so just pass in the descriptor we got from our call to pcap_open_live
     and an allocated struct pcap_pkthdr.
  */

  /* A loop is required here; if not, pcap_next() run too fast after the
     "descr" take control of the adpter. Before "descr" gets its first sweet
     packet, pcap_next() has already run over.
  */
  while(1)
  {
  ...
  */
  ...
  /* struct 'ether_header' is defined in net/ethernet.h and included in
     netinet/if_ether.h */
