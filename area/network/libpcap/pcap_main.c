 ...
 *
 * Description:
 * main program to test different call back functions to pcap_loop();
 *
 * Compile with:
                  ...
                  const u_char* packet)
{
  ...
  if (length < sizeof(struct my_ip)) {
    fprintf(stdout, "truncated ip %d", length);
    return NULL;
  }
                 ...
                 const u_char *packet)
{
  ...
  if (ntohs(type) == ETHERTYPE_IP) { /* handle IP packet */
    handle_IP(args, pkthdr, packet);
  } else if (ntohs(type) == ETHERTYPE_ARP) { /* handle arp packet */
    fprintf(stdout, "ARP\n");
  } else if (ntohs(type) == ETHERTYPE_REVARP) { /* handle reverse arp packet */
