...
//
// - The namespace API consists of three system calls—clone(), unshare(), and
//   setns() — and a number of /proc files.
// - As with most other namespaces (user namespaces are the exception), creating
//   a UTS namespace requires privilege (specifically, CAP_SYS_ADMIN).
...
//   the CLONE_NEW* bits in its flags argument and makes the caller a member of
//   the namespaces.
//
