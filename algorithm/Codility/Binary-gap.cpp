...
// contains twob binary gaps: one of length 4 and one of length 3. The number
...
int solution(int N) {
    ...
    while (N != 0) {
        int bit = N & 0x01;
        // if current bit is 0, we need to make sure we have met 1 before
        if (bit == 0 && start == true)
            cur_gap++;
        } else {
            if (start == false) { // first '1'
                start = true;
            } else {
