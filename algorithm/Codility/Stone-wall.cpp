...
// that, given a zero-indexed array H of N positive integers specifying the height
...
int solution(const vector<int> &H) {
    ...
    for (int i = 0; i < H.size(); i++) {
        // pop out until we find a previous block we can use
        while (!preBlocks.empty() && H[i] < preBlocks.top())
            preBlocks.pop();
        // if height match exactly, we don't have to use stone
        if (!preBlocks.empty() && preBlocks.top() == H[i]) {
            continue;
        } else {    // otherwise, use one more stone and push current height
            preBlocks.push(H[i]);
            count++;
        }
    }
