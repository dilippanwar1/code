...
// planes that come too close together, since this may indicate a possible
// collision.

// The Brute force solution is O(n^2), compute the distance between each pair
// and return the smallest. We can calculate the smallest distance in O(nLogn)
// time using Divide and Conquer strategy. In this post, a O(n x (Logn)^2)
// approach is discussed. We will be discussing a O(nLogn) approach in a separate
// post.
...
//    from P[0] to P[n/2]. The second subarray contains points from P[n/2+1]
//    to P[n-1].
...
//    need to consider the pairs such that one point in pair is from left half
//    and other is from right half. Consider the vertical line passing through
...
// 6) Find the smallest distance in strip[]. This is tricky. From first look,
//    it seems to be a O(n^2) step, but it is actually O(n). It can be proved
//    geometrically that for every point in strip, we only need to check at most
...
// Time Complexity
// Let Time complexity of above algorithm be T(n). Let us assume that we use
// a O(nLogn) sorting algorithm. The above algorithm divides all points in two
// sets and recursively calls for two sets. After dividing, it finds the strip
// in O(n) time, sorts the strip in O(nLogn) time and finally finds the closest
...
//    above algorithm. We will soon be discussing the optimized solution in a
//    separate post.
// 2) The code finds smallest distance. It can be easily modified to find the
//    points with smallest distance.
// 3) The code uses quick sort which can be O(n^2) in worst case. To have the
